...
| ☐ | **Print the four documents and look at them** | The print stylesheet is written and its rules verified in the CSSOM — A4, 20/18mm margins, a full point-size scale, orphans and widows, break rules — but nothing has been rendered to paper or PDF. `npm run pdf` does it and installs Puppeteer on demand. Check page breaks around entries and that the cover does not strand. |
