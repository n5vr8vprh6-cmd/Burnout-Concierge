...
/* Loaded per page rather than site-wide. scroll.js takes over the wheel and
   makes sections tall, and neither belongs anywhere a visitor is trying to
   finish a task — so it ships only where `tier: 'cinematic'` is declared. */
const PAGE_SCRIPTS = { intake: '/js/intake.js', cinematic: '/js/scroll.js' };
...
function head(page, site, v) {
  const ver = (p) => p + (v && v[p] ? `?v=${v[p]}` : '');
...
<link rel="stylesheet" href="${ver('/css/tokens.css')}">
<link rel="stylesheet" href="${ver('/css/chrome.css')}">
<link rel="stylesheet" href="${ver('/css/site.css')}">
<link rel="stylesheet" href="${ver('/css/intake.css')}">
${JS_FLAG}`;
}
...
/* Same versioning as the stylesheets — see assetVersions() in build.js. */
function versionedScript(src, versions) {
  return versions && versions[src] ? `${src}?v=${versions[src]}` : src;
}

function render(page, site, body, versions) {
  ...
  if (page.tier === 'cinematic') scripts.push(PAGE_SCRIPTS.cinematic);
...
${scripts.map((s) => `<script src="${versionedScript(s, versions)}" defer></script>`).join('\n')}
</body>
</html>
`;
}
