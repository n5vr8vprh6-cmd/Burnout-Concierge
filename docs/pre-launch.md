...
| ⚠ | **The prepared documents say one thing and behave another way** | All four are linked from every footer, all four are in the sitemap, none is `noindex`. Three close with *"Prepared for you by Burnout Concierge. Not for distribution."* — untrue of a page one click away and submitted to search. The Infrastructure Brief is the exception and says *"Circulate freely inside your organisation,"* which is right. Three ways out: **(a)** `noindex` the three and drop them from the footer, so the intake is the real door; **(b)** keep them public and change the line to match; **(c)** accept the tension knowingly. Not picked — it changes what the conversion model *is*. |
