...
/* LONGFORM assumes it owns the page, because on /privacy, /terms and
   /accessibility it does — one section, one h1. On /contact it is used twice
   alongside a pageHeader, which gave that page three h1 elements. Pass
   `level: 2` wherever longform is a section rather than the whole page. */
function longform(s) {
  ...
  <div class="wrap lf__wrap">
    <header class="lf__head" data-stagger>
      ${eyebrow(s.eyebrow)}
      <${s.level === 2 ? 'h2' : 'h1'} class="l" data-reveal>${s.headline}</${s.level === 2 ? 'h2' : 'h1'}>
      ${s.lead ? `<p class="lede" data-reveal>${s.lead}</p>` : ''}
