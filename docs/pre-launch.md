...
| | Item | Notes |
|---|---|---|
| ☑ | **Venue names** | Confirmed in writing and now on the homepage chapters, in `/collection` and in the journey data: Four Seasons Los Cabos at Costa Palmas, Six Senses Kyoto, Maxwell Residences Puerto Vallarta. Revo has none (no venue yet); Eclipse credits Saint Lucia WELL rather than naming a property. |
...
| ☑ | **The prepared documents are private again** | Collection, Advisor Prospectus and Property Brief are `noindex` and out of the footer, which the intake email already delivered. The Infrastructure Brief stays public — it says *"Circulate freely inside your organisation."* Note: `noindex` asks crawlers not to list them, it does not gate them; the URLs still resolve for anyone who has one. |
...
| ☑ | **`/thank-you` is intake-aware** | Seven real pages at `/thank-you/<intake>`, built from the same `confirm` block that feeds the email — a path rather than a query parameter, because the page exists for visitors with scripting off. `assertConfirmationsExist` fails the build if an intake ever loses its page. |
| ☑ | **The four documents print correctly** | Rendered with `npm run pdf` and inspected. A4, ordered type scale, no page opening mid-sentence, no stranded pages. Prospectus and Property Brief came down to 3pp, Collection 3pp, Infrastructure Brief 4pp. Puppeteer installs with `npm i --no-save puppeteer` — it is deliberately not a dependency. |
