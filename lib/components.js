...
/* AVIF is opt-in per image rather than assumed, because the spec only carries
   a base name and a list of widths — there is nothing in it that says which
   formats were actually written to disk. The AI panels that predate
   tools/images.js exist only as WebP, and emitting a <source> for a file that
   is not there costs every visitor a failed request before the fallback. Set
   `avif: true` on anything the image pipeline produced. */
function figure(img, opts = {}) {
  ...
  const tag = `<img src="${esc(img.src)}" srcset="${esc(set('webp'))}" sizes="${esc(sizes)}"
      width="${img.w}" height="${img.h}" alt="${esc(img.alt || '')}"
      loading="${loading}" decoding="async"${fetchpri}${drift}>`;

  if (!img.avif) return tag;
  return `<picture>
      <source type="image/avif" srcset="${esc(set('avif'))}" sizes="${esc(sizes)}">
      ${tag}
    </picture>`;
}
...
/* ── The pin scaffolding ─────────────────────────────────────────────────────
   ...
   One device today: the steps replace one another, one at a time. A section may
   only be pinned if its advancing children contain nothing focusable. Clearing a link from view while leaving it in the tab order is
   how you strand a keyboard user. */
const pinAttrs = (s, steps) =>
  s.pin === true ? ` data-pin style="--steps:${steps}"` : '';
...
function collection(s) {
  const panels = s.journeys.map((j, i) => {
    ...
    const flip = i % 2 === 1;
    return `
  <article class="journey ${dark ? 'is-dark' : 'is-light'}${flip ? ' is-flipped' : ''}${j.partner ? ' is-partner' : ''}">
    <div class="journey__media" data-wipe>${figure(j.img, { drift: true, sizes: j.partner ? '100vw' : '(max-width: 900px) 100vw, 56vw' })}</div>
...
/* ══════════════════════════════════════════════════════════════════════════
   4b · CHAPTERS — the four worlds
   ──────────────────────────────────────────────────────────────────────────
   The cinematic heart of the homepage, and the only place on the site where
   scroll is genuinely a timeline rather than a way down a page.

   ONE SECTION PER CHAPTER, NOT ONE SECTION OF FOUR
     js/scroll.js writes --p (0 to 1) onto each pinned SECTION as it travels
     through its own track. Four chapters therefore need four sections, or they
     would share one progress value and zoom in unison.

   THE ZOOM IS CSS, NOT JAVASCRIPT
     --p inherits, so the media can scale itself against it without this file
     or the engine knowing anything about images. That also means the zoom
     simply does not happen when the engine is not running, which is the
     behaviour we want rather than a special case we have to write.

     Chapter media must NOT take `drift`. js/motion.js writes transform
     directly onto [data-drift] images, and two systems writing one property
     is how you get an element that stutters or freezes.

   WHAT MAY BE PINNED
     The beats carry no links. The venue line and the way into each journey sit
     outside [data-step], visible and focusable at every scroll position — the
     precondition every pin on this site has had to meet.
   ══════════════════════════════════════════════════════════════════════════ */
function chapters(s) {
  /* A header before the sequence, and a coda after it. The coda is where Revo
     lives: an urban reset is a different kind of thing from a journey abroad,
     it is still in development, and giving it a cinematic chapter it cannot
     yet earn would be the one dishonest moment on the page. */
  const intro = `<section class="band is-dark band--deepest" id="${esc(s.id || '')}" data-label="${esc(s.label || '')}">
  <div class="wrap">
    ${eyebrow(s.eyebrow)}
    <h2 class="l" data-reveal>${s.headline}</h2>
    ${lede(s.lead)}
  ...
  const outro = s.coda ? `<section class="band is-dark band--deepest ch__coda" data-label="${esc(s.coda.label || '')}">
  <div class="wrap" data-stagger>
    <p class="meta" data-reveal>${esc(s.coda.mark)}</p>
    <h3 class="ch__h" data-reveal>${s.coda.head}</h3>
    <p class="ch__p" data-reveal>${s.coda.body}</p>
    <div class="acts" data-reveal>${btn(s.coda.cta, 'quiet', 'document_request')}</div>
  </div>
</section>` : '';

  const body = s.chapters.map((c, i) => {
    const beats = c.beats.map((b, bi) => `
          <div class="ch__beat" data-step="${bi}">
            <h3 class="ch__h">${b.head}</h3>
            <p class="ch__p">${b.body}</p>
          </div>`).join('');

    const body = `<div class="wrap ch__body">
        <p class="meta ch__mark">${esc(c.n)} &mdash; ${esc(c.place)}</p>
        <div class="ch__beats">${beats}
        </div>
        <div class="ch__foot">
          <p class="meta ch__venue">${esc(c.venue)}</p>
          ${c.cta ? `<a class="btn btn--quiet" href="${esc(c.cta.href)}"
             data-event="retreat_view" data-event-label="${esc(c.name)}">${esc(c.cta.label)} &rarr;</a>` : ''}
        </div>
      </div>`;

    const stage = `<div class="ch__media">${figure(c.img, {
        eager: i === 0, sizes: '100vw'
      })}</div>
      <div class="ch__scrim" aria-hidden="true"></div>
      ${body}`;

    return `<section class="band chapter is-dark band--pin ch--${esc(c.tone)}"
  id="${esc(c.id || '')}" data-label="${esc(c.place)}" data-pin style="--steps:${c.beats.length}">
...
function stages(s) {
  ...
  return `<section class="band ${skinClass(s)}${pin ? ' band--pin' : ''}" id="${esc(s.id || '')}" data-label="${esc(s.label || '')}"${pinAttrs(s, s.stages.length)}>
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
/* The h1 is the page title, minus the brand suffix every <title> carries.
   It is derived rather than restated so the two cannot drift: change the
   page title and the heading follows it. `spec.title` still names the
   document being prepared, which is what the progress rail label wants. */
function pageHeading(page, site, fallback) {
  if (!page || !page.title) return fallback;
  const suffix = site && site.name ? ' — ' + site.name : '';
  return suffix && page.title.endsWith(suffix)
    ? page.title.slice(0, -suffix.length)
    : page.title;
}

function intake(s, page, site) {
  ...
  <div class="wrap intake__wrap">
    ...
    <form class="intake__form" data-intake data-intake-name="${esc(s.intake)}"
      ...
      <header class="intake__head">
        <!-- h1, not a span. This is the page title: it matches <title>, and
             before it was marked up the only headings on an intake page were
             the footer's five column labels. -->
        <h1 class="meta intake__title">${esc(pageHeading(page, site, spec.title))}</h1>
      ...
      <!-- The DEFAULT state here is the scriptless one, and that inversion is
           the point. Send carries no hidden attribute, because without
           js/intake.js it
           is the only control that does anything; Continue and Back are the
           step engine's own buttons and start hidden because nothing can
           advance a step without it.

           This was the other way round, which meant a visitor with scripting
           off saw one working-looking button that did nothing and no way to
           submit at all — while /thank-you exists precisely to catch the
           native post that could therefore never happen. js/intake.js says in
           its own header that "hidden is only ever applied by script"; it was
           right, and the markup was not.

           show(0, false) runs at init and sets all three, so an enhanced page
           corrects this before anyone can click. -->
      <div class="intake__acts">
        <button class="btn btn--primary" type="button" data-next hidden>Continue</button>
    ...
    if (b.type === 'entry') {
      return `<article class="entry">
        <div class="entry__head">
          ${b.destination ? `<span class="meta entry__dest">${esc(b.destination)}</span>` : ''}
          <h3 class="entry__name">${esc(b.name)}</h3>
          ${b.venue ? `<p class="entry__venue">${esc(b.venue)}</p>` : ''}
        </div>
...
function numbered(s) {
  ...
  <div class="wrap">
    ...
    <h2 class="l" data-reveal>${s.headline}</h2>
    ${lede(s.lead)}
    <${s.ordered === false ? 'ul' : 'ol'} class="numbered${s.ordered === false ? ' numbered--plain' : ''}" data-stagger>${s.items.map((it, i) => {
      ...
      <li class="numbered__item" data-reveal>
        ${s.ordered === false ? '' : `<span class="meta numbered__n">${esc(it.marker || String(i + 1).padStart(2, '0'))}</span>`}
        <div>
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
/* LONGFORM assumes it owns the page, because on /privacy, /terms and
   /accessibility it does — one section, one h1, headings beneath it at h2.
   On /contact it is used twice alongside a pageHeader. Pass `level: 2` there
   and the whole heading tree shifts down one, not just the headline. */
function longform(s) {
  const toc = s.blocks.filter((b) => b.type === 'h' && b.id);

  /* Every heading inside shifts down one when this is a section rather than the
     page. Demoting only the headline, which is what the first pass did, left
     the body headings at h2 — the same level as the section headline that
     contains them — so the outline claimed they were siblings of their own
     parent. A screen reader navigating by level got a flat list where there is
     real nesting. */
  const d = s.level === 2 ? 1 : 0;
  const H = (n) => 'h' + (n + d);

  const body = s.blocks.map((b) => {
    if (b.type === 'h')    return `<${H(2)} class="lf__h" id="${esc(b.id || '')}">${esc(b.text)}</${H(2)}>`;
    if (b.type === 'sub')  return `<${H(3)} class="lf__sub">${esc(b.text)}</${H(3)}>`;
  ...
  <div class="wrap lf__wrap">
    <header class="lf__head" data-stagger>
      ${eyebrow(s.eyebrow)}
      <${H(1)} class="l" data-reveal>${s.headline}</${H(1)}>
      ${s.lead ? `<p class="lede" data-reveal>${s.lead}</p>` : ''}
      ${s.reviewed ? `<p class="meta lf__date" data-reveal>Last reviewed ${esc(s.reviewed)}</p>` : ''}
    </header>

    ${toc.length ? `<nav class="lf__toc" aria-label="On this page" data-reveal>
      <${H(2)} class="meta lf__toc-h">On this page</${H(2)}>
...
const RENDERERS = {
  hero, columns, rungs, collection, chapters, stages, ladder, pathways, ecosystem, founder, finalCta,
  intake, prose, confirmation,
  pageHeader, fit, faq, numbered, longform
};

/* Renderers receive the page and site they are being rendered into, so a
   section can derive something from the page rather than repeat it. Only
   `intake` needs this today — its h1 is the page title — and every other
   renderer simply ignores the extra arguments. */
function renderSections(sections, page, site) {
  return sections.map((s) => {
    const fn = RENDERERS[s.type];
    if (!fn) throw new Error(`Unknown section type "${s.type}" — add it to lib/components.js`);
    return fn(s, page, site);
  }).join('\n\n');
}
