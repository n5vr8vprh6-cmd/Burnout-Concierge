...
| ☐ | **`/thank-you` next-steps are consumer-flavoured** | Every intake redirects there, so a Studio applicant or a property owner reads *"a reply written for your circumstances"* and *"a conversation."* Making it intake-aware means carrying the key through the 303 — worth it only if the fallback turns out to be hit often. The page exists chiefly for scripting-off visitors, so a JS fix would miss them. |
